...
@implementation DCAttributedLabel
...
- (NSMutableAttributedString *)attributedString:(NSString *)text
    ...
    [attributedString addAttribute:NSFontAttributeName value:font range:NSMakeRange(0, attributedString.length)]; //展开收起设置粗体时问题
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    CGFloat height = 0;
    // 3.1绘制展开状态
    if (_state == kDCAttributedTextStatusOpen) {
        // 获取初始行数
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (self.isSelected  && self.numberOfLines >= 0) {
        self.isSelected = NO;
        // 更改当前状态
        if (self.state == kDCAttributedTextStatusClose) {
            self.state = kDCAttributedTextStatusOpen;
            ...
            self.state = kDCAttributedTextStatusClose;
